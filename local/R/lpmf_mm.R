lpmf_mm <- function(train,
  ...
  tr.rmse <- aux$rmse
  tr.mae <- aux$mae
  tr.loss <- loss_lpmf(train, t(U), t(V), sig2/sig2u, sig2/sig2v)
  #tr.sig2 <- sig2
  tr.sig2u <- sig2u
  tr.sig2v <- sig2v
  ...
  for(epoch in 1:max_iter){
    ...
    for(iter in 1:num_bat){
      ...
      # calculate gradient
      grad <- grad_lpmf(df, t(U), t(V), sig2/sig2u, sig2/sig2v, batch, nrow(train))
      grad_U <- t(grad$Ut)
      grad_V <- t(grad$Vt)
    ...
    loss <- loss_lpmf(train, t(U), t(V), sig2/sig2u, sig2/sig2v)
    aux <- pred_lpmf(test, t(U), t(V))
